...
from baseline.utils import get_metric_cmp, get_model_file
from eight_mile.tf.optz import EagerOptimizer
from baseline.tf.tfy import TRAIN_FLAG, SET_TRAIN_FLAG, get_shape_as_list, get_version, create_distribute_strategy
        ...
        with strategy.scope():
            ...
            def _distributed_train_no_state(inputs):
                ...
                return strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_loss, axis=None), strategy.reduce(
                    tf.distribute.ReduceOp.SUM, per_replica_toks, axis=None)

            @tf.function
            def _distributed_train_with_state(inputs, hidden):

                h, per_replica_loss, per_replica_toks = strategy.run(_replicated_train_step_with_state,
                                                                     args=(inputs, hidden,))
    ...
    def test(self, vs, reporting_fns, phase, steps=0):
        ...
        with strategy.scope():
            ...
            def _distributed_test_with_state(inputs, hidden):
                ...
                h, per_replica_loss, per_replica_toks = strategy.run(_replicated_test_step_with_state,
                                                                     args=(inputs, hidden,))
                step_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_loss, axis=None)
                step_toks = strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_toks, axis=None)
                return h, step_loss, step_toks
